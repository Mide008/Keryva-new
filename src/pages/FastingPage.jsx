    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <RevealCard>
        <div style={{ borderRadius: 24, overflow: 'hidden', position: 'relative', background: 'var(--ink-900)', padding: 28 }}>
          <img src={dailyBackgroundImage('fasting',900,600)} alt="" aria-hidden="true" style={{position:'absolute',inset:0,width:'100%',height:'100%',objectFit:'cover',opacity:0.07}}/>
          ...
          <div style={{ fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--gold-300)', marginBottom: 8 }}>{t('fastingTag')}</div>
          <MotionHeadline text={t('fastingHeadline')} as="h1" style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(20px,3vw,30px)', fontWeight: 400, color: 'rgba(250,247,242,0.95)', lineHeight: 1.2 }} />
      ...
      <div style={{ display: 'flex', gap: 0, background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderRadius: 12, padding: 4, width: 'fit-content' }}>
        {[['build', t('fastingTabBuild')], ['library', `${t('fastingTabSaved')} (${fastingEntries.length})`]].map(([m, label]) => (
        ...
        <>
          <RevealCard delay={0.05}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 8 }}>{t('fastTypeLabel')}</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {FAST_TYPES.map(f => (
                    <button key={f.key} onClick={() => setFastType(f.key)} className={`tag ${fastType === f.key ? 'tag-dark' : 'tag-ink'}`} style={{ cursor: 'pointer', padding: '7px 14px', fontSize: 13, fontWeight: fastType === f.key ? 600 : 400 }}>
                      {f.emoji} {tOpt('fastingType', f.label)}
                    </button>
                  ))}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', marginBottom: 8 }}>{t('durationLabel')}</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {DURATIONS.map(d => (
                    <button key={d} onClick={() => setDuration(d)} className={`tag ${duration === d ? 'tag-gold' : 'tag-ink'}`} style={{ cursor: 'pointer', padding: '7px 14px', fontSize: 13, fontWeight: duration === d ? 600 : 400 }}>
                      {tOpt('fastingDuration', d)}
                    </button>
                  ))}
                </div>
              </div>
              <div className="input-group">
                <label className="input-label">{t('fastGoalLabel')}</label>
                <textarea className="textarea-field" rows={3} placeholder={t('fastGoalPlaceholder')}
                  value={goal} onChange={e => setGoal(e.target.value)} />
              </div>
              <div className="grid-2" style={{ gap: 12 }}>
                <div className="input-group">
                  <label className="input-label">{t('scriptureTranslationLabelShort')}</label>
              ...
              <MagneticBtn onClick={generate} disabled={!goal.trim() || loading} className="btn btn-primary btn-lg" style={{ width: '100%', justifyContent: 'center', gap: 10 }}>
                {loading ? <><span className="loading-dots"><span className="loading-dot" /><span className="loading-dot" /><span className="loading-dot" /></span> {t('buildingFastingPlan')}</> : <>{t('getFastingPlanBtn')}</>}
              </MagneticBtn>
            </div>
          </RevealCard>
            ...
            {result && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div className="card-gold">
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--gold-700)', marginBottom: 8 }}>{t('purposeLabel')}</div>
                  <p style={{ fontFamily: 'var(--font-serif)', fontSize: 16, fontStyle: 'italic', color:'var(--text-primary)', lineHeight: 1.75 }}>{result.purpose}</p>
                </div>

                <div className="card-elevated">
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>{t('whatToExpectLabel')}</div>
                  <p style={{ fontSize: 14, color:'var(--text-secondary)', lineHeight: 1.7 }}>{result.whatToExpect}</p>
                </div>

                <div>
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>{t('scripturesToStandOn')}</div>
                ...
                <div className="card">
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>{t('dailyPrayerFocusLabel')}</div>
                ...
                <div className="card">
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>{t('practicalTipsLabel')}</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div className="card-dark">
                  <div style={{ fontSize: 11, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--gold-300)', marginBottom: 10 }}>{t('breakingTheFastLabel')}</div>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  <button onClick={save} className="btn btn-gold">{t('saveFastingPlanBtn')}</button>
                  <button onClick={shareWA} className="btn btn-outline">💬 WhatsApp</button>
                  <button onClick={() => { setResult(null); setGoal('') }} className="btn btn-ghost">{t('startOverLabel')}</button>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      {view === 'library' && (
        fastingEntries.length === 0
          ? <EmptyState icon="🍽" headline={t('noFastingPlansHeadline')} body={t('noFastingPlansBody')} ctaLabel={t('buildOneNowCta')} onCta={() => setView('build')} />
...
function FastingEntryCard({ entry: e, services, langLabel }) {
  ...
  const saveJournal = () => {
    if (!reflection.trim() && !lesson.trim() && !answeredPrayer.trim()) { showToast(t('writeSomethingFirst'), '⚠️'); return }
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>{e.date} · {e.fastType} · {e.duration}{e.completed ? ` · ✅ ${t('completedLabel')}` : ''}</div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, flexWrap: 'wrap' }}>
            ...
            {!e.completed && !loggedToday && <button onClick={() => logFastingDay(e.id)} className="btn btn-gold btn-sm" style={{ fontSize: 11.5, padding: '4px 10px' }}>{t('logTodayBtn')}</button>}
            {!e.completed && loggedToday && <span style={{ fontSize: 12, color: 'var(--sage-600)' }}>{t('todayLoggedLabel')}</span>}
            {!e.completed && <button onClick={() => setJournalOpen(o => !o)} className="btn btn-outline btn-sm" style={{ fontSize: 11.5, padding: '4px 10px' }}>{t('addReflectionBtn')}</button>}
          </div>
        </div>
        <button onClick={async () => { if (await confirmAction(t('deleteFastingPlanConfirm'), { tone: 'danger', confirmLabel: t('delete') })) deleteFastingEntry(e.id) }} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 16, color: 'var(--terra-400)' }}>🗑</button>
      </div>

      {journalOpen && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <textarea className="textarea-field" rows={2} placeholder={t('todaysReflectionPlaceholder')} value={reflection} onChange={e2 => setReflection(e2.target.value)} />
          <textarea className="textarea-field" rows={2} placeholder={t('lessonPlaceholder')} value={lesson} onChange={e2 => setLesson(e2.target.value)} />
          <input className="input-field" placeholder={t('answeredPrayerPlaceholder')} value={answeredPrayer} onChange={e2 => setAnsweredPrayer(e2.target.value)} />
          <button onClick={saveJournal} className="btn btn-gold btn-sm" style={{ alignSelf: 'flex-start' }}>{t('saveReflectionBtn')}</button>
        </div>
      )}

      {(e.journal || []).length > 0 && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('journalLabel')} ({e.journal.length})</div>
          {e.journal.slice(0, 5).map((j, i) => (
            <div key={i} style={{ fontSize: 13, color:'var(--text-secondary)', lineHeight: 1.6 }}>
              ...
              {j.lesson && <p style={{ color: 'var(--text-muted)' }}>{t('lessonPrefix')} {j.lesson}</p>}
              {j.answeredPrayer && <p style={{ color: 'var(--sage-600)' }}>{t('answeredPrefix')} {j.answeredPrayer}</p>}
            </div>
          ))}
        </div>
      )}
      ...
      {!e.completed && (e.daysCompleted || []).length > 0 && (
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)' }}>
          <button onClick={finishFast} disabled={reviewing} className="btn btn-primary btn-sm">
            {reviewing ? t('buildingReviewLabel') : t('completeFastReviewBtn')}
          </button>
        </div>
      )}
        ...
        <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-subtle)', background: 'var(--sage-100)', borderRadius: 12, padding: 16 }}>
          ...
          {endReview.answeredPrayers?.length > 0 && (
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--sage-600)', marginBottom: 4 }}>{t('answeredPrayersLabel')}</div>
