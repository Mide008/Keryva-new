...
const THEME_KEYS = {'Healing':'topicHealing','Finances':'topicFinancialBlessing','Protection':'themeProtection','Family':'themeFamily','Identity & Purpose':'topicIdentityInChrist','Peace of mind':'themePeaceOfMind','Favor':'themeFavor','Marriage':'topicMarriage','Career & Business':'themeCareerBusiness'}
...
export default function ConfessionsPage(){
  ...
  const generate = async () => {
    ...
    if (r) { setResult(r); showToast(t('declarationsReadyToast'), '🕊') }
          ...
          <RevealCard delay={0.05}>
            <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
              <div className="input-group">
                <label className="input-label">{t('focusAreaLabel')}</label>
              ...
              <div className="grid-2">
                <div className="input-group">
                  <label className="input-label">{t('translationLabel')}</label>
              ...
              <MagneticBtn onClick={generate} disabled={loading} className="btn btn-primary btn-lg" style={{ width:'100%', justifyContent:'center', gap:10 }}>
                {loading ? <><span className="loading-dots"><span className="loading-dot"/><span className="loading-dot"/><span className="loading-dot"/></span> {t('writingDeclarations')}</> : <>{t('generateDeclarationsBtn')}</>}
              </MagneticBtn>
            </div>
          </RevealCard>
              ...
              <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {result.declarations?.map((d,i)=>(
                  <div key={i} className="verse-card">
                    ...
                    <p style={{ fontFamily:'var(--font-serif)', fontSize:17, fontStyle:'italic', color:'var(--text-primary)', lineHeight:1.75, fontWeight:500 }}>"{d.text}"</p>
                  </div>
                ))}
