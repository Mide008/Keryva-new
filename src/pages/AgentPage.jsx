    ...
    try {
      ...
      recorder.onstop = async () => {
        ...
        try {
          ...
          else showToast(t('couldNotTranscribe'), '⚠️')
        } catch {
          showToast(t('voiceUnavailable'), '⚠️')
        }
...
{t('resumeOfferPrefix')} {resumeOffer.lastArtifact?.type} {t('resumeOfferSuffix')} "{resumeOffer.lastArtifact?.args?.topic || resumeOffer.lastArtifact?.args?.instruction || t('yourLastSessionLabel')}".
          </span>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={resumeDraft} className="btn btn-gold btn-sm">{t('continueItBtn')}</button>
            <button onClick={dismissResume} className="btn btn-ghost btn-sm">{t('startFreshBtn')}</button>
          </div>
        </div>
      )}
        ...
        {messages.map((m, i) => (
          <MessageBubble key={i} m={m} onSaveSermon={handleSaveSermon} onSaveStudy={handleSaveStudy} onSaveSunday={handleSaveSunday}
            ...
            onGoToBible={(ref) => {
              ...
              if (!m2) { showToast(t('couldNotOpenReference'), '⚠️'); return }
      ...
      <form onSubmit={handleSend} style={{ display: 'flex', gap: 8, marginTop: 14, position: 'relative' }}>
        ...
        {voiceSupported && (
          <button type="button" onClick={toggleListening} className="btn btn-outline" disabled={busy || transcribing}
            ...
            title={listening ? t('stopListening') : transcribing ? t('transcribingLabel') : t('speakInsteadOfTyping')}>
